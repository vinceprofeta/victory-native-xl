...
>({
  ...
  const prevViewportXRef = React.useRef<[number, number] | null>(viewport?.x || null)
  ...
  React.useLayoutEffect(() => {
    ...
    if (viewportWidth <= 0 || currentTotalContentWidth < 0) {
      console.log("Scroll Effect: Skipping due to invalid dimensions", { viewportWidth, currentTotalContentWidth })
    ...
    const viewportXChanged =
      viewport?.x && (prevViewportXRef?.current?.[0] !== viewport?.x?.[0] || prevViewportXRef?.current?.[1] !== viewport?.x?.[1])
    ...
    console.log(
      `Scroll Effect: Clamping. MaxScroll=${maxScroll.toFixed(2)}, CalcScroll=${newScrollX.toFixed(2)}, ClampedScroll=${clampedScrollX.toFixed(2)}`,
    )
  ...
  }, [dimensions.totalContentWidth, dimensions.width, data.length, scrollX, prevTranslateX, viewport?.x])
