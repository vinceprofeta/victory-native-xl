...
import type { InputDatum, InputFields, ValueOf, XAxisProps, XAxisPropsWithDefaults, ChartBounds } from "../../types"
import { runOnJS, type SharedValue, useDerivedValue, useSharedValue, useAnimatedReaction } from "react-native-reanimated"
...
export const XAxis = <RawData extends Record<string, unknown>, XK extends keyof InputFields<RawData>>({
  ...
  const xScale = useMemo(() => (zoom ? zoom.rescaleX(xScaleProp) : xScaleProp), [zoom, xScaleProp])
  const [y1 = 0, y2 = 0] = yScale.domain()
  const fontSize = font?.getSize() ?? 0

  // Use tickValues if provided, otherwise generate ticks
  const xTicksNormalized = useMemo(
    () => (tickValues ? downsampleTicks(tickValues, tickCount) : enableRescaling ? xScale.ticks(tickCount) : xScaleProp.ticks(tickCount)),
    [tickValues, tickCount, enableRescaling, xScale, xScaleProp],
  )

  // Ref to store the last reported visible ticks on the JS thread
  const lastReportedVisibleTicksRef = useRef<Array<ValueOf<RawData[XK]>> | null>(null)
  ...
  const processAndReportTicks = useMemo(() => {
    ...
    return () => {
      ...
      xTicksNormalized.forEach((tick) => {
        ...
        if (scrolledPixelX >= chartBounds.left && scrolledPixelX <= chartBounds.right) {
          const indexPosition = uniqueValueIndices.get(String(tick)) ?? tick
          const dataValue = (isNumericalData ? tick : ix[indexPosition as number]) as ValueOf<RawData[XK]>
      ...
      if (!isEqual(lastReportedVisibleTicksRef.current, actualVisibleData)) {
        console.log("actualVisibleData", actualVisibleData)
  ...
  useAnimatedReaction(
    () => {
      ...
      let firstVisibleTick: ValueOf<RawData[XK]> | number | string | null = null
    ...
    (current, previous) => {
      ...
      if (previous === null || current.first !== previous.first || current.last !== previous.last) {
        runOnJS(processAndReportTicks)()
      }
    },
    // Dependencies for the reaction prepare block
    [scrollX, scaleDomain, scaleRange, chartBounds, xTicksNormalized, processAndReportTicks],
  )
